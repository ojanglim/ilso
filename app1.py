...
if analyze_btn:
    with st.spinner(f"데이터 정밀 분석 중..."):
        ...
        4. factors: 다음 8가지 요인에 대한 각각의 구체적인 분석 내용을 포함한 객체, "~니다"와 같은 존댓말 사용
           ...
           - timing_expl: 현재 시기적 특성(계절적인 수요 언급 and 날짜에 따라 설 추석 등의 음식이 많이 필요한 명절 기간(명절의 1~2주 전후)이 다가오거나 지나간지 얼마 안됐다면 언급, 아니라면 언급 금지,또한 명절 기간이라도 명절에 수요가 많은 생산품이 아니라면 언급 금지)과 수요 변화 분석
        ...
        5. long_advice: 10개 이상의 상세 판매 전략 (각 전략의 첫문장은 굵게 처리하고 "~세요"와 같은 문장으로 끝맺음, '~을''~를'과 같은 조사 빼먹지 말것, 마침표 찍을 것, 첫문장 이후 관련 이모티콘 한개 사용, 첫문장이후 부가설명 작성, 전략마다 번호매김 필수, 전략마다 줄바꿈 두번 필수)
        """
